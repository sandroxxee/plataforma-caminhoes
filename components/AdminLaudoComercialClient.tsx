...
const motivos = ["Financiamento", "Venda particular", "Troca", "Consórcio", "Inventário", "Garantia", "Avaliação mercadológica"];
const fotosObrigatorias = ["Frente", "Traseira", "Lateral esquerda", "Lateral direita", "Interior", "Motor", "Pneus", "Documento", "Implemento", "Foto adicional"];
...
export function AdminLaudoComercialClient({ truck }: Props) {
  ...
  function toggleFoto(item: string) {
    setCheckFotos((current) => current.includes(item) ? current.filter((foto) => foto !== item) : [...current, item]);
  }
    ...
    <div style={styles.wrapper}>
      <section style={styles.formCard} className="no-print">
        ...
        <div style={styles.formGrid}>
          <label style={styles.label}>Valor comercial avaliado<input style={styles.input} value={valorAvaliado} onChange={(event) => setValorAvaliado(event.target.value)} /></label>
          <label style={styles.label}>Motivo da avaliação<select style={styles.input} value={motivo} onChange={(event) => setMotivo(event.target.value)}>{motivos.map((item) => <option key={item}>{item}</option>)}</select></label>
          <label style={styles.label}>Placa<input style={styles.input} value={placa} onChange={(event) => setPlaca(event.target.value)} /></label>
          <label style={styles.label}>RENAVAM<input style={styles.input} value={renavam} onChange={(event) => setRenavam(event.target.value)} /></label>
          <label style={styles.label}>Chassi<input style={styles.input} value={chassi} onChange={(event) => setChassi(event.target.value)} /></label>
          <label style={styles.label}>Estado de conservação<select style={styles.input} value={conservacao} onChange={(event) => setConservacao(event.target.value)}><option>Bom estado operacional, com sinais normais de uso</option><option>Muito bom estado de conservação</option><option>Excelente estado operacional</option><option>Regular, com necessidade de reparos</option></select></label>
        </div>

        <div style={styles.checkCard}>
          <strong style={styles.checkTitle}>Fotos conferidas no laudo</strong>
          <div style={styles.checkGrid}>{fotosObrigatorias.map((item) => <label key={item} style={styles.checkItem}><input type="checkbox" checked={checkFotos.includes(item)} onChange={() => toggleFoto(item)} /> {item}</label>)}</div>
        </div>

        <label style={styles.label}>Detalhes técnicos e comerciais<textarea style={styles.textarea} value={detalhes} onChange={(event) => setDetalhes(event.target.value)} /></label>
        <label style={styles.label}>Frase explicando o valor de mercado<textarea style={styles.textarea} value={justificativa} onChange={(event) => setJustificativa(event.target.value)} /></label>
        <label style={styles.label}>Documento do caminhão / CRLV / ATPV-e / comprovante<input style={styles.input} type="file" accept="image/*,.pdf" onChange={handleDocumento} /></label>
        <label style={styles.label}>Adicionar fotos manualmente ao laudo<input style={styles.input} type="file" accept="image/*" multiple onChange={handleFotos} /></label>

        <div style={styles.formGrid}>
          <label style={styles.label}>Responsável / empresa<input style={styles.input} value={responsavel} onChange={(event) => setResponsavel(event.target.value)} /></label>
          <label style={styles.label}>WhatsApp<input style={styles.input} value={whatsapp} onChange={(event) => setWhatsapp(event.target.value)} /></label>
        </div>
        <label style={styles.label}>Assinatura visual<input style={styles.input} value={assinatura} onChange={(event) => setAssinatura(event.target.value)} /></label>
      ...
      <section style={styles.sheet}>
        <header style={styles.sheetHeader}>
          <div><strong style={styles.brand}>Caminhões à Venda</strong><h1 style={styles.title}>Laudo Comercial de Avaliação Mercadológica</h1><p style={styles.subtitle}>Documento comercial para apoio em venda, troca, negociação ou análise de crédito.</p></div>
          <div style={styles.badge}>Avaliação<br />Comercial</div>
        </header>

        <div style={styles.sheetBody}>
          <div style={styles.valueBox}><div><span style={styles.valueLabel}>Valor comercial avaliado</span><p style={styles.valueText}>Baseado nas condições informadas, fotos, mercado e conjunto operacional.</p></div><strong style={styles.value}>{valorAvaliado}</strong></div>
          ...
          <div style={styles.photoChecklist}>{fotosObrigatorias.map((item) => <span key={item} style={checkFotos.includes(item) ? styles.checkedPill : styles.uncheckedPill}>{checkFotos.includes(item) ? "✓" : "○"} {item}</span>)}</div>

          <h2 style={styles.sectionTitle}>3. Documento do caminhão</h2>
          <div style={styles.documentBox}>{documento ? <img src={documento} alt="Documento do caminhão" style={styles.documentImage} /> : documentoNome ? <strong>Documento anexado: {documentoNome}</strong> : <span>Espaço reservado para CRLV, ATPV-e, comprovante ou documento do caminhão.</span>}</div>

          <h2 style={styles.sectionTitle}>4. Registro fotográfico</h2>
          <div style={styles.photoGrid}>{fotosLaudo.length > 0 ? fotosLaudo.map((foto, index) => <figure key={`${foto}-${index}`} style={styles.figure}><img src={foto} alt={`Foto do caminhão ${index + 1}`} style={styles.photo} /><figcaption style={styles.caption}>Foto {index + 1}</figcaption></figure>) : <div style={styles.emptyPhoto}>Nenhuma foto adicionada.</div>}</div>

          <h2 style={styles.sectionTitle}>5. Características e estado operacional</h2><p style={styles.paragraph}>{detalhes}</p>
          <h2 style={styles.sectionTitle}>6. Fundamentação do valor comercial</h2><p style={styles.paragraph}>{justificativa}</p>
          ...
          <div style={styles.signatureGrid}><div><div style={styles.signatureBox}>{assinatura}</div><div style={styles.signatureLine}><strong>{responsavel}</strong><br />Responsável pela avaliação comercial</div></div><div><div style={styles.blankSignature}></div><div style={styles.signatureLine}>Cliente / Proprietário<br />Assinatura ou ciência das informações</div></div></div>
          <footer style={styles.footer}><span>WhatsApp: {whatsapp}</span><span>www.caminhoesavenda.com.br</span></footer>
        </div>
      </section>
...
function Info({ label, value }: { label: string; value: string }) {
  return <div style={styles.infoBox}><span style={styles.infoLabel}>{label}</span><strong style={styles.infoValue}>{value}</strong></div>;
}
