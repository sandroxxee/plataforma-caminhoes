...
import { ChevronDown, ChevronRight, Info, LogIn, Menu, Search, Truck, X } from "lucide-react";
import { useState } from "react";

const truckLinks = [
  { href: "/anuncios", label: "Todos os caminhões", helper: "Lista completa" },
  { href: "/anuncios?modelo=Cavalo%20mec%C3%A2nico", label: "Cavalo mecânico", helper: "Conjuntos e cavalos" },
  { href: "/anuncios?modelo=Truck", label: "Truck", helper: "Caminhões truck" },
  { href: "/anuncios?modelo=Toco", label: "Toco", helper: "Caminhões toco" },
  { href: "/anuncios?carroceria=Prancha", label: "Prancha", helper: "Pranchas e plataformas" },
  { href: "/anuncios?carroceria=Chassis", label: "Chassis", helper: "Chassis disponíveis" },
  { href: "/anuncios?tracao=6x4", label: "6x4 / Traçado", helper: "Tração pesada" },
];
...
export function PublicHeader() {
  ...
  const trucksActive = pathname === "/anuncios" || pathname.startsWith("/anuncios/");
    ...
    <header className="public-header">
      <div className="public-header-shell">
        ...
        <nav className="public-nav" aria-label="Menu principal">
          <div className="nav-dropdown">
            <Link href="/anuncios" className={trucksActive ? "active dropdown-trigger" : "dropdown-trigger"} aria-current={trucksActive ? "page" : undefined}>
              <Truck size={16} aria-hidden="true" />
              <span>Caminhões</span>
              <ChevronDown size={14} aria-hidden="true" />
            </Link>
            <div className="dropdown-menu" aria-label="Submenu de caminhões">
              {truckLinks.map((link) => (
      ...
      <div className={`mobile-menu ${open ? "open" : ""}`} aria-hidden={!open}>
        ...
        <div className="mobile-menu-list">
          <Link href="/anuncios" onClick={closeMenu} className={trucksActive ? "active" : ""}>
            <span className="mobile-icon"><Truck size={18} aria-hidden="true" /></span>
            <span><strong>Caminhões</strong><small>Todos os caminhões</small></span>
            <ChevronRight size={17} aria-hidden="true" />
          </Link>

          <div className="mobile-submenu">
            {truckLinks.slice(1).map((link) => (
