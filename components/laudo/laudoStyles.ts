...
export const styles: Record<string, CSSProperties> = {
  ...
  formCard: { padding: 18, borderRadius: 22, background: "var(--surface)", border: "1px solid var(--line)", boxShadow: "var(--shadow)" },
  formTitle: { margin: 0, color: "var(--text)", fontSize: 22 },
  helpText: { margin: "8px 0 16px", color: "var(--muted)", lineHeight: 1.45 },
  formGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 },
  label: { display: "grid", gap: 7, marginTop: 12, color: "var(--text)", fontWeight: 800, fontSize: 13 },
  input: { width: "100%", border: "1px solid var(--line)", background: "var(--soft)", color: "var(--text)", borderRadius: 12, padding: 12, font: "inherit", outline: "none" },
  textarea: { width: "100%", minHeight: 105, border: "1px solid var(--line)", background: "var(--soft)", color: "var(--text)", borderRadius: 12, padding: 12, font: "inherit", resize: "vertical", outline: "none" },
  checkCard: { marginTop: 14, padding: 14, borderRadius: 16, background: "var(--soft)", border: "1px solid var(--line)" },
  checkTitle: { display: "block", marginBottom: 10, color: "var(--text)" },
  checkGrid: { display: "grid", gridTemplateColumns: "1fr 1fr", gap: 8 },
  checkItem: { color: "var(--muted)", fontSize: 13, fontWeight: 800 },
