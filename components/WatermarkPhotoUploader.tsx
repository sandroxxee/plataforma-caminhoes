...
type EditorTarget = { type: "principal" | "extra"; index: number; item: PreviewItem };
type AiAnalysis = {
  nota?: number;
  resumo?: string;
  problemas?: string[];
...
function addFineGrain(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
  const dots = Math.min(2200, Math.max(180, Math.round((w * h) / 520)));
  ctx.save();
  ...
  for (let i = 0; i < dots; i += 1) {
    ...
    ctx.fillRect(x + Math.random() * w, y + Math.random() * h, 1.15, 1.15);
  }
  ctx.restore();
}
...
function revokePreviews(items: PreviewItem[]) { items.forEach((item) => URL.revokeObjectURL(item.url)); }
function makePreviews(files: File[]) { return files.map((file) => ({ file, name: file.name, url: URL.createObjectURL(file) })); }
...
function normalizeRect(start: Point, end: Point): BlurRect { return { x: Math.min(start.x, end.x), y: Math.min(start.y, end.y), w: Math.abs(end.x - start.x), h: Math.abs(end.y - start.y) }; }
...
function BlurEditor({ target, onCancel, onApply }: { target: EditorTarget; onCancel: () => void; onApply: (type: EditorTarget["type"], index: number, file: File) => void }) {
  ...
  async function apply() {
    if (rectsRef.current.length === 0) { setStatus("Selecione pelo menos uma área para borrar antes de aplicar."); return; }
    ...
    } catch (error) { console.error(error); setStatus("Não consegui aplicar o desfoque. Tente selecionar uma área menor."); }
  }

  function undoLast() { const nextRects = rectsRef.current.slice(0, -1); rectsRef.current = nextRects; setRects(nextRects); drawCanvas(nextRects); }
  function clearRects() { rectsRef.current = []; setRects([]); drawCanvas([]); }

  return (
    <div className="editor-backdrop" role="dialog" aria-modal="true"><div className="editor-panel"><div className="editor-head"><div><strong>Borrar placa/contato</strong><p>{status}</p></div><button type="button" onClick={onCancel} className="editor-close">Fechar</button></div><div className="mode-actions"><button type="button" onClick={() => setStatus("Automático com IA será a próxima etapa. Agora use o manual para garantir.")}>Automático</button><button type="button" className="mode-active">Manual: arrastar caixa</button></div><canvas ref={canvasRef} className="blur-canvas" onPointerDown={startDrag} onPointerMove={moveDrag} onPointerUp={endDrag} onPointerCancel={endDrag} /><div className="editor-actions"><button type="button" onClick={undoLast} disabled={rects.length === 0}>Desfazer última área</button><button type="button" onClick={clearRects} disabled={rects.length === 0}>Limpar marcações</button><button type="button" onClick={apply} className="apply-blur">Aplicar desfoque</button></div></div><style jsx>{`.editor-backdrop{position:fixed;inset:0;z-index:9999;padding:18px;display:grid;place-items:center;background:rgba(0,0,0,.78);backdrop-filter:blur(10px)}.editor-panel{width:min(1040px,100%);max-height:96vh;overflow:auto;border-radius:22px;border:1px solid rgba(255,255,255,.16);background:#071014;padding:16px;box-shadow:0 30px 80px rgba(0,0,0,.45)}.editor-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;margin-bottom:12px}.editor-head strong{display:block;color:white;font-size:18px;margin-bottom:5px}.editor-head p{margin:0;color:#cbd5e1;line-height:1.45}.editor-close,.editor-actions button,.mode-actions button{min-height:42px;border:1px solid rgba(255,255,255,.14);border-radius:12px;background:rgba(255,255,255,.08);color:white;padding:0 14px;font-weight:900;cursor:pointer}.mode-actions{display:flex;gap:10px;flex-wrap:wrap;margin-bottom:12px}.mode-active{background:#22c55e!important;color:#052e16!important;border-color:transparent!important}.blur-canvas{width:100%;height:auto;border-radius:16px;border:1px solid rgba(255,255,255,.14);background:rgba(2,6,23,.78);touch-action:none;cursor:crosshair;display:block;user-select:none}.editor-actions{display:flex;gap:10px;flex-wrap:wrap;justify-content:flex-end;margin-top:14px}.editor-actions button:disabled{opacity:.45;cursor:not-allowed}.apply-blur{background:#22c55e!important;color:#052e16!important;border-color:transparent!important}`}</style></div>
  );
}
...
export function WatermarkPhotoUploader() {
  ...
  function previewKey(type: EditorTarget["type"], index: number) { return `${type}-${index}`; }
  function updateInputs(principalItems: PreviewItem[], extraItems: PreviewItem[]) { setInputFiles(principalRef.current, principalItems.map((item) => item.file)); setInputFiles(extrasRef.current, extraItems.map((item) => item.file)); }

  function removePhoto(type: EditorTarget["type"], index: number) {
    if (type === "principal") {
      setPrincipalPreview((old) => { const next = old.filter((item, itemIndex) => { if (itemIndex === index) URL.revokeObjectURL(item.url); return itemIndex !== index; }); updateInputs(next, extrasPreview); return next; });
      setStatus("Foto principal removida. Você pode escolher outra antes de enviar.");
      return;
    }
    setExtrasPreview((old) => { const next = old.filter((item, itemIndex) => { if (itemIndex === index) URL.revokeObjectURL(item.url); return itemIndex !== index; }); updateInputs(principalPreview, next); return next; });
    setStatus("Foto extra removida.");
  }

  async function analyzePhoto(type: EditorTarget["type"], index: number, item: PreviewItem) {
    ...
    try {
      ...
      const response = await fetch("/api/fotos/analisar-ia", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ imageDataUrl }) });
  ...
  async function processPrincipal(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setPrincipalPreview((old) => { revokePreviews(old); updateInputs(nextPrincipal, extrasPreview); return nextPrincipal; });
      setStatus("Marca d’água aplicada. Agora você pode analisar com IA, borrar placa/contato ou remover a foto.");
    } catch (error) { console.error(error); setStatus("Não consegui aplicar a marca d’água nessa foto. Tente outra imagem."); } finally { setProcessando(false); }
  }
  ...
  async function processExtras(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      setExtrasPreview((old) => { revokePreviews(old); updateInputs(principalPreview, nextExtras); return nextExtras; });
      setStatus(`Marca d’água aplicada em ${processed.length} foto${processed.length === 1 ? "" : "s"}. Use a IA para escolher capa e corrigir enquadramento.`);
    } catch (error) { console.error(error); setStatus("Não consegui aplicar a marca d’água em uma das fotos. Tente enviar menos imagens ou fotos menores."); } finally { setProcessando(false); }
  }

  function applyEditedFile(type: EditorTarget["type"], index: number, file: File) {
    const nextItem = makePreviews([file])[0];
    if (type === "principal") {
      setPrincipalPreview((old) => { const next = [...old]; if (next[index]) URL.revokeObjectURL(next[index].url); next[index] = nextItem; updateInputs(next, extrasPreview); return next; });
    } else {
      setExtrasPreview((old) => { const next = [...old]; if (next[index]) URL.revokeObjectURL(next[index].url); next[index] = nextItem; updateInputs(principalPreview, next); return next; });
    }
    setEditorTarget(null);
    setStatus("Desfoque aplicado com cantos arredondados, sem cor e com granulação fina. Confira a prévia antes de enviar.");
  }

  const allPreviews = [...principalPreview.map((item, index) => ({ item, index, type: "principal" as const, label: "Principal" })), ...extrasPreview.map((item, index) => ({ item, index, type: "extra" as const, label: "Extra" }))];

  return (
    <div className="watermark-uploader"><div className="photo-grid"><label className="upload-field"><strong>Foto principal</strong><small>Marca d’água, IA, desfoque de placa/contato e remoção antes de enviar.</small><input ref={principalRef} name="foto_principal" type="file" accept="image/*" onChange={processPrincipal} disabled={processando} /></label><label className="upload-field"><strong>Fotos extras</strong><small>Frente, traseira, lateral, cabine, pneus e carroceria.</small><input ref={extrasRef} name="fotos_extras" type="file" accept="image/*" multiple onChange={processExtras} disabled={processando} /></label></div>{status && <p className="watermark-status">{status}</p>}{allPreviews.length > 0 && <div className="preview-grid-watermark">{allPreviews.map(({ item, index, type, label }) => { const key = previewKey(type, index); const analise = analises[key]; return <figure key={`${item.name}-${type}-${index}`}><img src={item.url} alt={item.name} /><figcaption>{label}</figcaption><div className="photo-actions"><button type="button" className="ai-button" onClick={() => analyzePhoto(type, index, item)} disabled={analisandoKey === key}>{analisandoKey === key ? "Analisando..." : "Analisar foto com IA"}</button><button type="button" className="blur-button" onClick={() => setEditorTarget({ type, index, item })}>Borrar automático/manual</button><button type="button" className="remove-button" onClick={() => removePhoto(type, index)}>Remover foto</button></div>{analise && <div className="ai-result"><strong>Nota da foto: {analise.nota ?? "-"}/10</strong>{analise.resumo && <p>{analise.resumo}</p>}{analise.melhorUso && <small>Melhor uso: {analise.melhorUso}</small>}{analise.problemas?.length ? <ul>{analise.problemas.map((item, i) => <li key={`p-${i}`}>{item}</li>)}</ul> : null}{analise.recomendacoes?.length ? <ul>{analise.recomendacoes.map((item, i) => <li key={`r-${i}`}>{item}</li>)}</ul> : null}{analise.corteSugerido?.explicacao && <p><b>Corte sugerido:</b> {analise.corteSugerido.explicacao}</p>}{analise.areasSensiveis?.length ? <p><b>Atenção:</b> {analise.areasSensiveis.map((a) => `${a.tipo}: ${a.descricao}`).join("; ")}</p> : null}</div>}</figure>; })}</div>}{editorTarget && <BlurEditor target={editorTarget} onCancel={() => setEditorTarget(null)} onApply={applyEditedFile} />}<style jsx>{`.watermark-uploader{display:grid;gap:14px}.photo-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.upload-field{min-height:150px;padding:20px;border-radius:20px;border:1px dashed rgba(34,197,94,.45);background:rgba(34,197,94,.07);align-content:center;display:grid;gap:8px;color:#dbeafe;font-size:13px;font-weight:900}.upload-field strong{font-size:18px;color:white}.upload-field small{color:#94a3b8;line-height:1.45;font-weight:700}.upload-field input{margin-top:8px;padding:12px;border-style:solid;background:rgba(2,6,23,.52);width:100%;min-height:50px;border-radius:15px;border:1px solid rgba(255,255,255,.14);color:white;box-sizing:border-box}.watermark-status{margin:0;color:#d9f99d;font-size:13px;line-height:1.45;font-weight:850}.preview-grid-watermark{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.preview-grid-watermark figure{margin:0;overflow:hidden;border-radius:16px;border:1px solid rgba(255,255,255,.12);background:rgba(2,6,23,.52);position:relative;display:grid}.preview-grid-watermark img{width:100%;max-height:520px;object-fit:contain;display:block;background:rgba(2,6,23,.78)}.preview-grid-watermark figcaption{position:absolute;left:8px;top:8px;padding:5px 8px;border-radius:999px;background:rgba(2,6,23,.75);color:white;font-size:11px;font-weight:950}.photo-actions{display:grid;grid-template-columns:1fr;gap:8px;margin:10px}.ai-button,.blur-button,.remove-button{min-height:42px;border:0;border-radius:12px;font-weight:950;cursor:pointer}.ai-button{background:rgba(59,130,246,.95);color:white}.blur-button{background:#22c55e;color:#052e16}.remove-button{background:rgba(239,68,68,.92);color:white}.ai-button:disabled{opacity:.55;cursor:not-allowed}.ai-result{margin:0 10px 12px;padding:12px;border-radius:14px;border:1px solid rgba(59,130,246,.32);background:rgba(30,64,175,.16);color:#dbeafe;display:grid;gap:7px;font-size:13px;line-height:1.45}.ai-result strong{color:white}.ai-result p,.ai-result ul{margin:0}.ai-result ul{padding-left:18px}.ai-result small{color:#bfdbfe;font-weight:850}@media(max-width:980px){.photo-grid,.preview-grid-watermark{grid-template-columns:1fr}}`}</style></div>
  );
}
