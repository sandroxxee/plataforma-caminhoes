      ...
      <div className="market-container">
        ...
        <div className="seo-marca-nav">
          {Object.entries(MARCA_DISPLAY).map(([slug, name]) => (
            <Link key={slug} href={`/caminhoes/${slug}`} className={slug === marca ? "active" : ""}>{name}</Link>
          ))}
        </div>
        ...
        <div className="seo-internal-links">
          ...
          <div className="seo-internal-row">
            {["sc","pr","rs","sp","mg","rj","go","ba"].map((uf) => (
              <Link key={uf} href={`/anuncios/estado/${uf}`}>{display} {uf.toUpperCase()}</Link>
            ))}
          </div>
        </div>
      </div>
