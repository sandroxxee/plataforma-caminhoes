  ...
  return (
    <main className="home-page">
      ...
      <section className="wrap home-hero">
        <div className="hero-card" style={heroImage ? { backgroundImage: `linear-gradient(105deg, rgba(2,7,5,.96), rgba(2,7,5,.82) 48%, rgba(2,7,5,.38)), linear-gradient(180deg, rgba(2,7,5,.18), rgba(2,7,5,.84)), url(${heroImage})` } : undefined}>
          <div className="hero-content">
            <span className="mini">Caminhões reais • Dados claros • WhatsApp direto</span>
          ...
          <div className="hero-showcase" aria-label="Anúncio em destaque">
            <span className="showcase-label">Destaque do estoque</span>
            <strong>{heroTruck ? getCardTitle(heroTruck) : "Caminhão em destaque"}</strong>
            <small>{heroTruck ? `${heroTruck.ano_modelo || heroTruck.ano_fabricacao || "Ano"} • ${heroTruck.tracao || heroTruck.carroceria || "Configuração"} • ${getLocation(heroTruck)}` : "Os anúncios aprovados aparecem aqui automaticamente."}</small>
            <b>{heroTruck ? formatMoney(heroTruck.preco) : "Sob consulta"}</b>
          </div>
        </div>
      ...
      <style>{`
        ...
        .hero-card{position:relative;overflow:hidden;min-height:318px;border-radius:30px;background:linear-gradient(115deg,rgba(7,17,13,.96),rgba(7,17,13,.72) 54%,rgba(34,211,125,.26));background-size:cover;background-position:center;border:1px solid var(--site-line,rgba(255,255,255,.13));box-shadow:var(--site-shadow,0 28px 80px rgba(0,0,0,.36));display:grid;grid-template-columns:minmax(0,1.1fr) minmax(280px,.62fr);align-items:end;gap:20px;padding:30px}
        ...
        .hero-showcase{position:relative;z-index:2;align-self:stretch;display:flex;flex-direction:column;justify-content:flex-end;gap:8px;min-height:210px;padding:20px;border-radius:24px;background:rgba(255,255,255,.09);border:1px solid rgba(255,255,255,.13);backdrop-filter:blur(14px);box-shadow:0 18px 44px rgba(0,0,0,.25)}.showcase-label{width:max-content;max-width:100%;padding:7px 10px;border-radius:999px;background:rgba(34,211,125,.16);border:1px solid rgba(34,211,125,.25);color:#bbf7d0;font-size:11px;font-weight:950;text-transform:uppercase;letter-spacing:.06em}.hero-showcase strong{font-size:clamp(23px,2.5vw,34px);line-height:1;letter-spacing:-.045em}.hero-showcase small{color:rgba(238,250,243,.78);font-weight:800;line-height:1.4}.hero-showcase b{color:#86efac;font-size:25px;line-height:1;margin-top:4px}
        ...
        .categories-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:14px}.category-card{border-radius:24px;border:1px solid var(--site-line);background:var(--site-surface);overflow:hidden;box-shadow:var(--site-shadow-soft);text-decoration:none;color:var(--site-text);transition:.2s ease}.category-card:hover{transform:translateY(-4px)}.category-photo{position:relative;min-height:122px;background:radial-gradient(circle at 72% 26%,color-mix(in srgb,var(--site-green) 30%,transparent),transparent 30%),linear-gradient(145deg,#1b2a23,#070f0b);display:grid;place-items:end start;padding:14px;color:#fff;font-weight:950}.category-photo::after{content:"";position:absolute;inset:auto 0 0 0;height:70%;background:linear-gradient(transparent,rgba(0,0,0,.55))}.category-photo span{position:relative;z-index:2}.category-card>div:last-child{padding:15px}.category-card strong{display:block;font-size:18px}.category-card p{margin:6px 0 0;color:var(--site-muted);font-size:13px;line-height:1.4;font-weight:750}
