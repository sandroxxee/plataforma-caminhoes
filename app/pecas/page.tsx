...
export const metadata: Metadata = {
  ...
  description: "Peças para caminhão: motores, câmbios, eixos, suspensão, freios e muito mais.",
...
export default async function PecasPage({ searchParams }: PageProps) {
  ...
  let query = supabase
    ...
    .eq("status", "aprovado").eq("perfil", "Peças")
    .order("created_at", { ascending: false }).limit(48);
  if (estadoFiltro) query = query.eq("estado", estadoFiltro);

  const { data } = await query;
  let pecas = (data || []) as TruckCardData[];
  if (marca) pecas = pecas.filter(t => t.marca === marca);
    ...
    <div className="pec-page">
      ...
      <div className="pec-cta">
        <div className="pec-cta-inner">
          <div>
            ...
            <p className="pec-sub">Motores, câmbios, eixos, suspensão, freios e muito mais. Contato direto pelo WhatsApp.</p>
          </div>
      ...
      <div className="pec-container">
        ...
        <div className="pec-estados">
          <Link href="/pecas" className={`pec-est-chip${!estadoFiltro ? " pec-est-chip--active" : ""}`}>Todos</Link>
          {ESTADOS.map(uf => (
            <Link key={uf} href={`/pecas?estado=${uf}`} className={`pec-est-chip${estadoFiltro === uf ? " pec-est-chip--active" : ""}`}>{uf}</Link>
          ))}
        </div>
        {pecas.length > 0 ? (
          <>
          ...
          <EmptyState
            ...
            description={estadoFiltro ? `Não há peças em ${estadoFiltro} no momento.` : "Nenhuma peça disponível no momento."}
            primaryHref="/pecas"
      ...
      <style>{`
        ...
        .pec-est-chip { display: inline-flex; align-items: center; justify-content: center; height: 30px; padding: 0 12px; border-radius: 999px; border: 1.5px solid var(--line); background: var(--surface); color: var(--muted); font-size: 12px; font-weight: 800; text-decoration: none; transition: border-color .12s, color .12s; }
        .pec-est-chip:hover, .pec-est-chip--active { border-color: var(--blue); color: var(--blue); background: var(--blueSoft); }
