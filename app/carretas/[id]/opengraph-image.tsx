...
export const alt         = "Carreta à venda";
...
export default async function OgImage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const modelo = item?.modelo || item?.titulo || "Carreta";
  ...
  return new ImageResponse(
    (
      <div style={{ width: 1200, height: 630, display: "flex", background: "#050d18", position: "relative", overflow: "hidden", fontFamily: "system-ui, sans-serif" }}>
        ...
        <div style={{ position: "absolute", left: 0, top: 0, bottom: 0, width: 6, background: "linear-gradient(to bottom, #f59e0b, #d97706)" }} />
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", justifyContent: "space-between", padding: "44px 56px 44px 62px" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 44, height: 44, borderRadius: 12, background: "linear-gradient(135deg,#f59e0b,#d97706)", boxShadow: "0 2px 12px rgba(245,158,11,.5)" }}>
            ...
            <div style={{ color: "#fff", fontWeight: 900, fontSize: 22, display: "flex", letterSpacing: -0.3 }}>
              Caminhões <span style={{ color: "#fbbf24", marginLeft: 6 }}>à Venda</span>
            </div>
          </div>
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {marca && (
              <div style={{ display: "flex" }}>
                <div style={{ display: "flex", padding: "5px 16px", borderRadius: 999, background: "rgba(245,158,11,0.18)", border: "1.5px solid rgba(251,191,36,0.4)", color: "#fbbf24", fontWeight: 900, fontSize: 18, letterSpacing: 1 }}>
                  {marca.toUpperCase()}
                </div>
              </div>
            )}
            <div style={{ color: "#fff", fontWeight: 900, fontSize: tituloSize, lineHeight: 1.08, display: "flex", letterSpacing: -1.5, maxWidth: 720 }}>
              {titulo || "Carreta à venda"}
            </div>
            {local && (
          ...
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "14px 28px", borderRadius: 16, background: "linear-gradient(135deg,#f59e0b,#d97706)", boxShadow: "0 4px 20px rgba(245,158,11,.45)" }}>
