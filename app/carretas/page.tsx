...
export const metadata: Metadata = {
  ...
  description: "Veja carretas e semirreboques à venda: graneleiras, porta-containers, pranchas, frigoríficas, tanques e muito mais.",
...
export default async function CarretasPage({ searchParams }: PageProps) {
  ...
  const { count: total } = await supabase
    .from("trucks").select("*", { count: "exact", head: true })
    .eq("status", "aprovado").eq("perfil", "Carretas");
    ...
    <div className="car-page">
      ...
      <div className="car-container">
        ...
        <div className="car-estados">
          ...
          {ESTADOS.map(uf => (
            <Link key={uf} href={`/carretas?estado=${uf}`} className={`car-est-chip${estadoFiltro === uf ? " car-est-chip--active" : ""}`}>{uf}</Link>
          ))}
        </div>

        {carretas.length > 0 ? (
          <>
            <p className="car-count">{carretas.length}{!estadoFiltro && (total ?? 0) > 24 ? `+ de ${total}` : ""} carreta{carretas.length !== 1 ? "s" : ""}</p>
          ...
          <EmptyState
            ...
            description={estadoFiltro ? `Não há carretas em ${estadoFiltro} no momento.` : "Nenhuma carreta disponível no momento."}
      ...
      <style>{`
        ...
        .car-est-chip { display: inline-flex; align-items: center; justify-content: center; height: 30px; padding: 0 12px; border-radius: 999px; border: 1.5px solid var(--line); background: var(--surface); color: var(--muted); font-size: 12px; font-weight: 800; text-decoration: none; transition: border-color .12s, color .12s; }
        .car-est-chip:hover, .car-est-chip--active { border-color: var(--blue); color: var(--blue); background: var(--blueSoft); }
