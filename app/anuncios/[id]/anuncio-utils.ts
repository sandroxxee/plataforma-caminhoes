...
export const PERFIL_TEXTOS: Record<string, {
  sobre: string; whatsapp: string; ficha: string; veiculo: string; emoji: string;
}> = {
  Máquinas:    { sobre: "Sobre esta máquina",    whatsapp: "Tenho interesse nesta máquina",    ficha: "Ficha técnica",   veiculo: "máquina",    emoji: "🏗️" },
  Peças:       { sobre: "Sobre esta peça",        whatsapp: "Tenho interesse nesta peça",        ficha: "Especificações", veiculo: "peça",        emoji: "🔩" },
  Carretas:    { sobre: "Sobre esta carreta",     whatsapp: "Tenho interesse nesta carreta",     ficha: "Ficha técnica",   veiculo: "carreta",     emoji: "🚚" },
  Implementos: { sobre: "Sobre este implemento",  whatsapp: "Tenho interesse neste implemento",  ficha: "Especificações", veiculo: "implemento",  emoji: "📦" },
};

const DEFAULT_TEXTOS = {
  sobre: "Sobre este caminhão", whatsapp: "Tenho interesse neste caminhão",
  ficha: "Ficha técnica", veiculo: "caminhão", emoji: "🚛",
};
...
export function getWhatsappLink(truck: Truck, title: string) {
  ...
  const artigo = textos.veiculo !== "caminhão"
    ? `n${["a", "e"].includes(textos.veiculo[0]) ? "a" : "o"} `
    : "no ";
  const text = encodeURIComponent(
    `Olá, tenho interesse ${artigo}${textos.veiculo} ${title}${
      truck.ano_modelo ? ` ano ${truck.ano_modelo}` : ""
...
export function getStructuredData(truck: Truck, title: string, location: string, whatsappLink: string) {
  ...
  const description    = truck.descricao?.trim() ||
  ...
  const vehicle = {
    ...
    name: title, description, url, image: imageAbsolute,
    ...
    mileageFromOdometer: truck.quilometragem
      ? { "@type": "QuantitativeValue", value: truck.quilometragem, unitCode: "KMT" } : undefined,
    ...
    potentialAction: whatsappLink
      ? { "@type": "ContactAction", target: whatsappLink, name: "Contato pelo WhatsApp" } : undefined,
  };
  ...
  const breadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "Anúncios", item: `${siteUrl}/anuncios` },
      { "@type": "ListItem", position: 3, name: truck.marca || "Anúncio",
        item: `${siteUrl}/anuncios?marca=${encodeURIComponent(truck.marca || "")}` },
