...
const PERGUNTAS: Record<string, string> = {
  tipo:          "Ola! Vamos criar seu anuncio em poucos minutos\n\nO que voce esta vendendo?\n\n**1.** Caminhao\n**2.** Carreta\n**3.** Implemento\n**4.** Maquina",
  ...
  descricao:     "Conte mais sobre o veiculo\n\nDescreva o **estado de conservacao**, **opcionais**, **historico de manutencao**, **km revisado**, etc.\n\n_Quanto mais detalhes, mais chances de vender!_",
};
...
function gerarResumo(dados: Record<string, string>, qtdFotos: number): string {
  ...
  return [
    "**Confira o resumo do seu anuncio:**",
    "",
    `Tipo: ${dados.tipo}`,
    ...
    `Fotos: ${qtdFotos} foto(s) enviada(s)`,
    `Descricao: ${dados.descricao?.slice(0, 100)}...`,
    "",
    "---",
    "Digite **publicar** para enviar para aprovacao ou **corrigir [campo]** para ajustar algo.",
    "_(Ex: \"corrigir preco\", \"corrigir cidade\", \"corrigir fotos\")_",
  ].join("\n");
}
...
export async function POST(req: NextRequest) {
  ...
  // Etapa fotos: aguarda continuar
  if (etapa === "fotos") {
    if (qtdFotos === 0) {
      return NextResponse.json({
        resposta: "Voce ainda nao enviou nenhuma foto. Por favor envie pelo menos 1 foto antes de continuar.",
        dados, etapa: "fotos",
      });
    }
  ...
  if (etapa === "confirmar") {
    ...
    if (resp.startsWith("corrigir")) {
      ...
      const mapa: Record<string, string> = {
        ...
        km: "km", preco: "preco", preco2: "preco",
      ...
      if (campo) {
        dados[campo] = "";
        if (campo === "fotos") {
          return NextResponse.json({ resposta: "Ok! Envie as novas fotos abaixo e clique em Continuar.", dados, etapa: "fotos" });
        }
        return NextResponse.json({ resposta: PERGUNTAS[campo], dados, etapa: campo });
      }
      return NextResponse.json({
        resposta: "Nao entendi qual campo corrigir. Ex: \"corrigir preco\", \"corrigir cidade\"",
    ...
    if (["publicar", "sim", "s", "ok", "confirmar", "enviar"].includes(resp)) {
      ...
      return NextResponse.json({
        resposta: "**Anuncio enviado para aprovacao!**\n\nEm breve nossa equipe ira revisar e publicar no site.\nAcompanhe o status no [seu painel](/painel).\n\n_Obrigado por anunciar no Caminhoes a Venda!_",
  ...
  if (etapa && PERGUNTAS[etapa]) {
    ...
    if (etapa === "tipo") {
      ...
      if (etapa === "preco" && Number(num) < 1000)
        return NextResponse.json({ resposta: "Valor muito baixo. Informe o preco em reais, ex: **320000**", dados, etapa });
      dados[etapa] = num;
    } else if (etapa === "ano_fabricacao" || etapa === "ano_modelo") {
      const ano = Number(val.replace(/\D/g, ""));
      if (ano < 1970 || ano > new Date().getFullYear() + 1)
        return NextResponse.json({ resposta: "Ano invalido. Informe o ano corretamente, ex: **2019**", dados, etapa });
      dados[etapa] = String(ano);
    } else if (etapa === "whatsapp") {
      const wpp = val.replace(/\D/g, "");
      if (wpp.length < 10 || wpp.length > 13)
        return NextResponse.json({ resposta: "WhatsApp invalido. Informe com DDD, ex: **47999990000**", dados, etapa });
      dados.whatsapp = wpp;
  ...
  if (proxima === "fotos") {
    return NextResponse.json({ resposta: PERGUNTAS.descricao ? gerarResumo(dados, 0) : "", dados, etapa: "fotos",
      respostaFotos: "Otimo! Agora envie as **fotos do veiculo** pelo botao abaixo.\n\n_Minimo 1 foto, recomendado pelo menos 5 fotos._\n\nQuando terminar, clique em **Continuar**.",
    });
  }
